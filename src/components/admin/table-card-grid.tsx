...
export function TableCardGrid({ cards, onStatusChange, onDeleteOrder, onCompleteTable, disabled }: TableCardGridProps) {
  return (
    <section className="admin-card-grid" data-testid="admin-table-card-grid">
      {cards.map((card) => (
        <article className="admin-table-card" data-testid={`admin-table-${card.tableNumber}-card`} key={card.tableId}>
